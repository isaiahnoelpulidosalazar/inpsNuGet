    ...
    public class TestPyCS
    {
        ...
        private void CreatePython()
        {
            if (!File.Exists(PythonZip))
            {
                ...
                try
                {
                    using (var ResourceStream = Assembly.GetExecutingAssembly().GetManifestResourceStream($"inpsNuGet.{PythonZip}"))
                    {
            ...
            if (!File.Exists(PythonFilesZip))
            {
                try
                {
                    using (var ResourceStream = Assembly.GetExecutingAssembly().GetManifestResourceStream("inpsNuGet.py_files.zip"))
                    {
                        ...
                        using (var FileStream = File.Create(PythonFilesZip))
                        {
                            ResourceStream.CopyTo(FileStream);
                        }
                    }
            ...
            if (ZipReadable)
            {
                ...
                if (!Directory.Exists(NestedExtractPath))
                {
                    ...
                    try
                    {
                        ...
                        if (File.Exists(PythonFilesZip))
                        {
                            ExtractZipSafe(PythonFilesZip, PythonDir);
                        }
                    }
                    catch (Exception e)
                    {
